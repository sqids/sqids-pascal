...
uses
  SysUtils, TestFramework, sqids, common;
...
const
  Numbers: TNumbers = [1,2,3];
  Pairs: array[0..11] of TMinLenIdPair = (
    (MinLen: 6;  Id: '86Rf07'),
    (MinLen: 7;  Id: '86Rf07x'),
...
begin
  for Pair in Pairs do
    ...
    try
      CheckEquals(Encode(Numbers), Pair.Id);
      CheckEquals(Encode(Numbers).Length, Pair.MinLen);
      CheckTrue(Decode(Pair.Id).Equals(Numbers));
    finally
      Free;
    end;
end;
...
initialization
  ...
  RegisterTest(TTestMinLength.Suite);
end.
